(() => {
  ...
  [
    Elements.color1Input,
    Elements.color2Input,
  ...
  function setupEventListeners() {
    ...
    function showSettingsChangedNotification() {
      const notification2 = document.getElementById("settingsChangedNotification");
      if (!notification2) {
        return;
      }
      ...
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          notification2.style.transform = "translateY(10px)";
          notification2.style.opacity = "1";
        });
      });
    }
    function hideSettingsChangedNotification() {
      const notification2 = document.getElementById("settingsChangedNotification");
      if (!notification2) return;
      notification2.style.transform = "translateY(-80px)";
      notification2.style.opacity = "0";
      notification2.addEventListener("transitionend", () => {
    ...
    Elements.dividerWidthInput.addEventListener(
      "input",
      (e) => Elements.dividerWidthValue.textContent = e.target.value + "%"
    );
    ...
    document.querySelectorAll('.custom-checkbox input[type="checkbox"]').forEach((cb) => {
      ...
      cb.addEventListener("DOMContentLoaded", updateClass);
    });
    ...
    Elements.exportBtn.addEventListener("click", () => {
      ...
      const cameras2 = getCameraTableData();
      const exportData = {
    ...
    async function disableAllStreamsSequentially() {
      ...
      const enabledToggles = toggles.filter((toggle) => {
        const cameraBox = toggle.closest(".camera-box");
        return toggle.checked && cameraBox && !cameraBox.classList.contains("camera-disconnected") && !cameraBox.classList.contains("dammy");
      });
      ...
      try {
        for (const toggle of enabledToggles) {
          const cameraBox = toggle.closest(".camera-box");
          if (!document.body.contains(toggle) || !toggle.checked || !cameraBox || cameraBox.classList.contains("camera-disconnected") || cameraBox.classList.contains("dammy")) continue;
    ...
    Elements.refreshCamerasBtn.onclick = () => {
      const allNotifications = document.querySelectorAll(".notification");
      allNotifications.forEach((notification2) => {
        if (notification2.dataset.collapsed === "true") {
          delete notification2.dataset.collapsed;
          notification2.classList.remove("collapsed");
        }
      });
      const notificationActions2 = document.getElementById("notificationActions");
      if (notificationActions2) {
        notificationActions2.classList.add("hidden");
      }
  ...
  const openBtn = Elements.openBtn;
  const modal = Elements.modal;
  const modalHolder = Elements.modalHolder;
  ...
  function recordPos() {
    ...
    Elements.tbody.querySelectorAll("tr").forEach((r) => m.set(r, r.getBoundingClientRect().top));
    return m;
  }
  ...
  function startPrinterStatusPolling(card, address) {
    ...
    const poll = async () => {
      try {
        ...
        if (!stopped) timer = setTimeout(poll, 5e3);
      }
    };
    void poll();
    const cleanup = () => {
  ...
  function savePrinterMac(rowIdx, mac, ip = "") {
    ...
    cameras[rowIdx].lastMacCheckAt = (/* @__PURE__ */ new Date()).toISOString();
    saveToLocalStorage();
    return true;
  }
  ...
  function closeModal() {
    ...
    const modal2 = document.getElementById("cameraSettingsModal");
    if (!modal2) return;
    document.body.focus();
    const focusableElements = modal2.querySelectorAll('button, input, select, textarea, [tabindex]:not([tabindex="-1"])');
    focusableElements.forEach((el) => el.blur());
    modal2.classList.add("invisible", "opacity-0");
    modal2.setAttribute("aria-hidden", "true");
    const contentDiv = modal2.querySelector(".flex-1");
  ...
  function renderCameras() {
    ...
    let orderedCameras = [...cameras2];
    if (savedSettings && savedSettings.cameraOrder) {
      const orderMap = {};
      savedSettings.cameraOrder.forEach((cam, index) => {
        orderMap[cam.ip] = index;
      });
      orderedCameras.sort((a, b) => (orderMap[a.ip] ?? Infinity) - (orderMap[b.ip] ?? Infinity));
    }
    let loadedCount = 0;
    const totalCameras = cameras2.length;
    if (totalCameras === 0) {
      hideLoader();
      return;
    }
    ...
    function enterFullscreen(cameraElement) {
      ...
      cameraElement.requestFullscreen().then(() => {
        isFullscreen = true;
    ...
    function openModal(camera) {
      ...
      const firstFocusable = modal2.querySelector("button, input, [tabindex]");
      if (firstFocusable) firstFocusable.focus();
      isModalOpen = true;
      const expandBtn = modal2.querySelector(".expand-icon");
      const manageBtn = modal2.querySelector(".manage-icon");
      ...
      const closeBtn = modal2.querySelector(".manage-icon");
    ...
    function stopAllVideoStreams() {
      ...
      allImages.forEach((img) => {
        ...
        if (img.src && !img.src.startsWith("data:") && (!streamToggle || streamToggle.checked)) {
    ...
    orderedCameras.forEach((cam, idx) => {
      ...
      camDiv.addEventListener("dragstart", (e) => {
        const isDammy2 = camDiv.dataset.ip === "dammy";
        camDiv.style.opacity = isDammy2 ? "0.6" : "0.5";
        camDiv.classList.add("dragging");
        e.dataTransfer.effectAllowed = "move";
      });
      camDiv.addEventListener("dragend", function() {
        const isDammy2 = this.dataset.ip === "dammy";
        const isDisconnected = this.classList.contains("camera-disconnected");
        if (isDammy2) this.style.opacity = "0";
  ...
  function getDividerGradient(startAlpha = "77", endAlpha = "E6") {
    const color = Elements.dividerColorInput.value;
    const color1 = Elements.color1Input.value;
    const color2 = Elements.color2Input.value;
  ...
  function updateHorizontalDividers() {
    ...
    for (let i = 1; i < rows; i++) {
      ...
      Object.assign(divider.style, {
        ...
        right: "auto",
        // Отключаем right, чтобы работало left
      ...
      divider.addEventListener("mouseleave", () => {
        ...
        const key2 = `row_${divider.dataset.rowIndex}`;
        const dividerStates2 = JSON.parse(localStorage.getItem("dividerVisibility") || "{}");
        const newStates = { ...dividerStates2, [key2]: newOpacity !== 0 };
  ...
  let activeNotifications = /* @__PURE__ */ new Set();
  const notificationBuffer = /* @__PURE__ */ new Set();
  ...
  const activeNotificationsBuffer = /* @__PURE__ */ new Set();
  ...
  function checkAndRemoveDuplicate(message) {
    ...
    if (existing) {
      animateAndRemoveNotification(existing);
      return true;
    }
    return false;
  }
  ...
  function updateAllNotificationPositions() {
    ...
    if (!container) return;
    const gap = 10;
    const notifications = Array.from(container.querySelectorAll(".notification")).filter((n) => !n.dataset.animating && n.dataset.collapsed !== "true").sort((a, b) => parseInt(a.dataset.number) - parseInt(b.dataset.number));
    if (notifications.length === 0) return;
    let cumulativeBottom = 0;
    notifications.forEach((notification, index) => {
      const newNumber = index;
      notification.dataset.number = newNumber;
      notification.style.zIndex = 40 + newNumber;
  ...
  function _createNotification(message, type = "info") {
    ...
    setTimeout(() => {
      const container = document.getElementById("notificationContainer");
      if (!container) return;
      const allNotifications = Array.from(container.querySelectorAll(".notification")).filter((n) => n.dataset.animating !== "true");
      ...
      const visibleCount = hasCollapsed ? Math.min(1, allNotifications.length) : allNotifications.filter((n) => n.dataset.collapsed !== "true").length;
  ...
  function startGlobalCollapseTimer() {
    ...
    }, NOTIFICATION_COLLAPSE_DELAY);
  }
  const NOTIFICATION_COLLAPSE_DELAY = 5e3;
  function collapseNotification(notificationElement) {
    if (!notificationElement || notificationElement.dataset.collapsed === "true" || notificationElement.closest(".camera-disconnected")) return;
    const container = document.getElementById("notificationContainer");
    const allNotifications = Array.from(container.querySelectorAll(".notification"));
  ...
  function saveToLocalStorage() {
    ...
    const orderedCameras = [...cameras2].sort((a, b) => (orderMap[a.ip] || 0) - (orderMap[b.ip] || 0));
    const settings = {
  ...
  function loadStreamToggleState(ip) {
    if (!State.allowLocalStorage) return true;
    const settings = JSON.parse(localStorage.getItem("printerCamsV2") || "{}");
    return settings.streamToggles && settings.streamToggles[ip] !== void 0 ? settings.streamToggles[ip] : true;
  }
  ...
  const observeri = new IntersectionObserver((e) => e.forEach((i) => i.isIntersecting && i.target.classList.add("show")), { threshold: 0.15 });
  document.querySelectorAll(".reveal").forEach((el) => observeri.observe(el));
  const aBtn = document.getElementById("allowConsentBtn");
  const dBtn = document.getElementById("denyConsentBtn");
  const aHint = document.getElementById("acceptHint");
  const dHint = document.getElementById("declineHint");
  const cBtn = document.getElementById("consetModalCloseBtn");
  const cHint = document.getElementById("closeHint");
  const hide = () => {
    aHint.style.opacity = 0;
    dHint.style.opacity = 0;
    cHint.style.opacity = 0;
  };
  aBtn.onmouseenter = () => {
    hide();
    aHint.style.opacity = 1;
  };
  dBtn.onmouseenter = () => {
    hide();
  ...
  document.getElementById("consetModalBtn")?.addEventListener("click", () => {
    const modal2 = document.getElementById("consentModal");
    toggleSections(false);
    if (modal2) {
