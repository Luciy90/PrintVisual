...
const defaultNetworkScannerDeps = {
...
function resolveNetworkScannerDeps(overrides = {}) {
    return {
        ...defaultNetworkScannerDeps,
        ...overrides
    };
}
export async function scanNetwork(input, overrides = {}) {
    ...
    for (const subnet of subnets) {
        ...
        const subnetResults = await runConcurrent(ips, input.concurrency ?? 24, ip => scanDevice(ip, input.probeTimeoutMs ?? 700, input.macTimeoutMs ?? 1000, deps));
        results.push(...subnetResults.filter(device => device.reachable));
    }
    return results;
}
export async function buildRecoveryPlan(input, overrides = {}) {
    const deps = resolveNetworkScannerDeps(overrides);
...
async function scanDevice(ip, probeTimeoutMs, macTimeoutMs, deps) {
    try {
...
function buildMacDiscoveryMap(devices) {
...
function matchSavedPrintersByMac(printers, macMap) {
    const matches = [];
    printers.forEach((printer, index) => {
        const mac = normalizeMac(printer.mac);
        if (!mac || printer.ip.toLowerCase() === "dammy")
            return;
        const device = macMap.get(mac);
        if (!device || device.conflict)
            return;
